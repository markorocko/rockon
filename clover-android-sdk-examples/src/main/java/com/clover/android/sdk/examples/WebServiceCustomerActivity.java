...
public class WebServiceCustomerActivity extends Activity {

  private static final String TAG = WebServiceCustomerActivity.class.getSimpleName();
  ...
  @Override
  /**
   * Class initially gets Instantiated, user account retrieved, then waits for button click
   * Place listener on mButton so when clicked will fire OnClickListener, which method populates
   */
  protected void onCreate(Bundle savedInstanceState) {
    Log.v(TAG, "xxxOnCreate "+CloverAccount.BUILD);

    super.onCreate(savedInstanceState);
    setContentView(R.layout.web_service_customer);  //Point to layout XML
  ...
  /**
   * OnClicklistener is activated when the "Issue web service request" is clicked
   */
  private View.OnClickListener mRequestOnClickListener = new View.OnClickListener() {
    @Override
    public void onClick(View v) {
      Log.v(TAG, "xxxOnClickListener "+CloverAccount.BUILD);
      queryWebService();
      v.setEnabled(false);
    }
  };
  ...
  private void queryWebService() {
    Log.v(TAG, "xxxqueryWebService "+CloverAccount.BUILD);
    new AsyncTask<Void, String, Void>() {
      ...
      protected Void doInBackground(Void... params) {
        try {
          Log.v(TAG, "xxx doInBackground "+CloverAccount.BUILD);
          Log.v(TAG, "Requesting auth token");
          CloverAuth.AuthResult authResult = CloverAuth.authenticate(WebServiceCustomerActivity.this, mAccount);
          //publishProgress("Authenticated as " + mAccount + ".  authToken=" + authResult.authToken + ", authData=" + authResult.authData);
          publishProgress("Authenticated as name:" + mAccount.name + " type:" + mAccount.type);
          ...
          if (authResult.authToken != null && authResult.baseUrl != null) {
            ...
            Log.v(TAG,"requesting merchant id using: " + url+
                    "received merchant id: " + merchantId+
                    "requesting inventory items using: " + url+
                    "received inventory items response: " + result);
  ...
  static class CustomHttpClient extends DefaultHttpClient {
    ...
    public static CustomHttpClient getHttpClient() {
      Log.v(TAG, "xxx CustomHttpClient "+CloverAccount.BUILD);
    ...
    public String post(String url, String body) throws IOException, HttpException {
      Log.v(TAG, "xxx post url:"+url+" body:"+body);
      String result;
